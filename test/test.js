...
var iterSqrt1pm1 = require( './../lib' );


// VARIABLES //

var rand = randu( 0.0, 100.0 );
...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof iterSqrt1pm1, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object', function test( t ) {
	...
	it = iterSqrt1pm1( uniform( 0.0, 100.0 ) );
...
tape( 'the function returns an iterator protocol-compliant object (value+done)', function test( t ) {
	...
	it = iterSqrt1pm1( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object which computes `sqrt(1+x) - 1` for each iterated value', function test( t ) {
	var expected;
	var rand;
	var it;
	var N;
	...
	N = 101;
	rand = uniform( 0.0, 100.0, {
		'iter': N
	});
	it = iterSqrt1pm1( rand );
	t.equal( it.next.length, 0, 'has zero arity' );
	...
	for ( i = 0; i < N; i++ ) {
		r = it.next();
		expected = sqrt1pm1( x.next().value );
...
tape( 'the function returns an iterator protocol-compliant object which computes `sqrt(1+x) - 1` for each iterated value (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	expected = [
		{
			'value': sqrt1pm1( values[ 0 ] ),
			'done': false
		},
		{
			'value': sqrt1pm1( values[ 1 ] ),
			'done': false
		},
		{
			'value': sqrt1pm1( values[ 2 ] ),
			'done': false
		},
		{
			'value': sqrt1pm1( values[ 3 ] ),
			'done': true
		}
	];
